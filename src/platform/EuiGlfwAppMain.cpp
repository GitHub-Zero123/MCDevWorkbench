// Project-owned EUI GLFW integration. Window lifecycle policy belongs here so
// the EUI submodule remains unmodified and independently upgradable.
#ifdef _WIN32
...
struct WindowState : app::AppRunner {
    ...
    bool maximizedKnown = false;
    int deferredFullPaintFrames = 0;
...
std::function<void()>& mainWindowRefreshHandler() {
    static std::function<void()> handler;
    return handler;
}

GLFWwindow*& mainWindowRefreshTarget() {
    static GLFWwindow* target = nullptr;
    return target;
}

void requestMainWindowRefresh(GLFWwindow* window, const bool force) {
    static bool refreshing = false;
    static bool refreshPending = false;
    static double lastRefreshTime = -1.0;
    std::function<void()>& handler = mainWindowRefreshHandler();
    if (!handler || window != mainWindowRefreshTarget()) {
        return;
    }
    if (refreshing) {
        refreshPending = true;
    ...
    constexpr double kInteractiveResizeInterval = 1.0 / 30.0;
    if (!force && lastRefreshTime >= 0.0
        && now - lastRefreshTime < kInteractiveResizeInterval) {
        return;
    }
...
void installWindowCallbacks(GLFWwindow* window, WindowState& windowState) {
    ...
    glfwSetFramebufferSizeCallback(window, [](GLFWwindow* currentWindow, int w, int h) {
        ...
        if (w > 0 && h > 0) {
            app::detail::requestFullPaint();
            requestMainWindowRefresh(currentWindow, false);
        }
    });
    ...
    glfwSetWindowMaximizeCallback(window, [](GLFWwindow* currentWindow, int maximized) {
        ...
        state->maximizedKnown = true;
        state->deferredFullPaintFrames = 2;
        app::detail::requestFullPaint();
        requestMainWindowRefresh(currentWindow, true);
        glfwPostEmptyEvent();
    });
...
int main() {
    ...
    const auto runMainWindowFrame = [&](const double currentFrameTime) {
        int framebufferWidth = 0;
        int framebufferHeight = 0;
        glfwGetFramebufferSize(window, &framebufferWidth, &framebufferHeight);
        if (framebufferWidth <= 0 || framebufferHeight <= 0) {
    ...
    mainWindowRefreshTarget() = window;
    mainWindowRefreshHandler() = [&] {
        if (windowState.hiddenToTray || windowState.iconified) {
            return;
        }
        const double now = glfwGetTime();
        windowState.paintRequested = true;
        windowState.nextFrameTime = now;
        app::detail::requestFullPaint();
        (void)runMainWindowFrame(now);
    };
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (!windowState.maximizedKnown) {
            windowState.maximized = maximized;
            windowState.maximizedKnown = true;
        } else if (windowState.maximized != maximized) {
            windowState.maximized = maximized;
            windowState.deferredFullPaintFrames = 2;
            windowState.paintRequested = true;
            app::detail::requestFullPaint();
        }
        ...
        if (anyAnimating || windowState.deferredFullPaintFrames > 0) {
            glfwPollEvents();
        } else {
            glfwWaitEvents();
        }
    ...
    mainWindowRefreshHandler() = {};
    mainWindowRefreshTarget() = nullptr;
